...
const SalesforceComponents = ['aura:', 'force:', 'forceChatter:', 'lightning:', 'ui:']
...
export function componentFactory(params = {}, type = DefaultComponentAdapter) {
    ...
    let adapterName = type.toLowerCase().replace('markup://', '').replace('c:', '');
    let adapter = ComponentAdapters[adapterName];
    if (!adapter) {
        if (!_.some(SalesforceComponents, name => adapterName.startsWith(name))) {
            /*eslint no-console: 0*/
